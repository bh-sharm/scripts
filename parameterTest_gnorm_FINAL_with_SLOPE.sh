...
# This includes the change in GNORM value of MOPAC
...
echo -e "paramNUM\t GNORM\t -0.005\t -0.004\t -0.003\t -0.002\t -0.001\t 0.000\t 0.001\t 0.002\t 0.003\t 0.004\t 0.005" >>OODimer.out
echo -e "paramNUM\t GNORM\t -0.005\t -0.004\t -0.003\t -0.002\t -0.001\t 0.000\t 0.001\t 0.002\t 0.003\t 0.004\t 0.005" >>OH1Monomer.out
echo -e "paramNUM\t GNORM\t -0.005\t -0.004\t -0.003\t -0.002\t -0.001\t 0.000\t 0.001\t 0.002\t 0.003\t 0.004\t 0.005" >>OH2Monomer.out
echo -e "paramNUM\t GNORM\t -0.005\t -0.004\t -0.003\t -0.002\t -0.001\t 0.000\t 0.001\t 0.002\t 0.003\t 0.004\t 0.005" >>costhetaMonomer.out
echo -e "paramNUM\t GNORM\t -0.005\t -0.004\t -0.003\t -0.002\t -0.001\t 0.000\t 0.001\t 0.002\t 0.003\t 0.004\t 0.005" >>bindingEnergyDimer.out
echo -e "paramNUM\t GNORM\t -0.005\t -0.004\t -0.003\t -0.002\t -0.001\t 0.000\t 0.001\t 0.002\t 0.003\t 0.004\t 0.005" >>angleBisector.out
echo -e "paramNUM\t GNORM\t -0.005\t -0.004\t -0.003\t -0.002\t -0.001\t 0.000\t 0.001\t 0.002\t 0.003\t 0.004\t 0.005" >>TotalEnergyMonomer.out

# loop for PARAMETERS #32
for num in {1..32};do
...
j=$(echo "$j-0.00" | bc)
sed "s/GNORM=1.0/GNORM=$j/g" monomer_bak.mop >monomer.mop
sed "s/GNORM=1.0/GNORM=$j/g" dimer_bak.mop >dimer.mop

# To print GNORM value in the output file
paramNum=$(awk 'NR=='$num' {print $1}' PARAM)
printf "%s \t" $paramNum $j >>OODimer.out
printf "%s \t" $paramNum $j >>OH1Monomer.out
printf "%s \t" $paramNum $j >>OH2Monomer.out
printf "%s \t" $paramNum $j >>costhetaMonomer.out
printf "%s \t" $paramNum $j >>bindingEnergyDimer.out
printf "%s \t" $paramNum $j >>angleBisector.out
printf "%s \t" $paramNum $j >>TotalEnergyMonomer.out


# Loop for parameter values change
i=$(echo "-0.006" | bc)
for a in {0..10};do
...
sed "s/$var1/$var2/g" PARAM >PARAMMOD
...
TotalEnergyLineMonomer=$(sed -n '/TOTAL ENERGY            /=' monomer.out)
TotalEnergyMonomerEV=$(awk 'FNR == '$TotalEnergyLineMonomer' {print $4}' monomer.out)
TotalEnergyMonomer=`echo "(23.061 * $TotalEnergyMonomerEV)" | bc`
...
# FOR FITTING
printf "%s \n" $OO >>fit_OODimer.out
printf "%s \n" $OH1 >>fit_OH1Monomer.out
printf "%s \n" $OH2 >>fit_OH2Monomer.out
printf "%s \n" $costheta >>fit_HOHMonomer.out
printf "%s \n" $bindingEnergy >>fit_bindingEnergyDimer.out
printf "%s \n" $cosAngleBisector >>fit_OOMdimer.out
printf "%s \n" $TotalEnergyMonomer >>fit_TotalEnergyMonomer.out
# FOR FITTING
...
printf "%s \t" $TotalEnergyMonomer >>TotalEnergyMonomer.out
...
printf "\n" >>angleBisector.out
printf "\n" >>TotalEnergyMonomer.out

# python script run

python fitting_script/fitTotalEnergyMonomer.py >>slopeTotalEnergyMonomer.out
python fitting_script/fitOH1Monomer.py >>slopeOH1Monomer.out
python fitting_script/fitOH2Monomer.py >>slopeOH2Monomer.out
python fitting_script/fitOODimer.py >>slopeOODimer.out
python fitting_script/fitBindingDimer.py >>slopeBindingDimer.out
python fitting_script/fitHOHangleMonomer.py >>slopeHOHangleMonomer.out
python fitting_script/fitOOMangledimer.py >>slopeOOMangledimer.out


done
# norm loop
# for line break after norm and increment
printf "\n" >>OODimer.out
printf "\n" >>OH1Monomer.out
printf "\n" >>OH2Monomer.out
printf "\n" >>costhetaMonomer.out
...
rm fit_OODimer.out fit_OH1Monomer.out fit_OH2Monomer.out fit_bindingEnergyDimer.out fit_TotalEnergyMonomer.out fit_HOHMonomer.out  fit_OOMdimer.out 
